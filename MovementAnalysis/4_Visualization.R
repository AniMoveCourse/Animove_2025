...
library(units)
library(lubridate)
#remotes::install_github("16EAGLE/moveVis") #(see below)
library(moveVis)
...
## only "osm" background 
ggplot() +
  ggspatial::annotation_map_tile(zoom = 9) + #add background maps
  ggspatial::annotation_scale(aes(location="br")) + #add scale bar
  theme_linedraw() +
...
##     - go to dashboard - manage properties authentication - API Key
## register api key:
...
dir.create("animationFld")
animate(
  animation,
  nframes = length(unique(data_interpolated$timestamp)),
  renderer = gifski_renderer("animationFld/animation.gif")
)
...
# we recently contributed some improvements that are not yet included in the 
## moveVis github repository above. You can access these improvements (speed of 
## function align_move, and quality of maps) by installing the libraries from
## these forked repositories
## remotes::install_github("robe2037/basemaps@tile-res") 
## remotes::install_github("robe2037/moveVis@moveapps")

library(moveVis)
library(move2)

# we will use the example dataset in the package, of storks in germany
data("move_data", package = "moveVis") # 'move_data' is a move2 class object
...
frames <- frames_spatial(
  m, path_colours = c("blue", "cyan", "purple"), high_res = TRUE,
...
# animate frames
animate_frames(frames, out_file = "animationFld/moveVis.gif")

# Link to the Animove 2022 recording from Jakob Schwalb-Willmann:
# https://animove.org/elearning/ -> 2022 -> moveVis
# Link to the website:
# https://movevis.org/
