#########################################################
###                   AniMove 2025                    ###
### Script by K. Safi, Martina Scacco & Anne Scharf   ###
#########################################################
###                    Space Use                      ###
#########################################################
...
setwd("/home/kami/Documents/Teaching/Animove/2025CR/data")
...
plot(data.frame(X330)[,c("coords.x1", "coords.x2")], type="n", xlab="Longitude", ylab="Latitude", pch=16, asp=1)
...
plot(bats_sp.proj[bats_sp.proj$id=="X021",], bty="na", xlab="Longitude", ylab="Latitude", pch=16)
...
bats_mv2.proj <- st_transform(bats_mv2, "EPSG:21781")
ind191 <- filter_track_data(bats_mv2.proj, .track_id = "191")

expand_factor <- 1.2   # e.g. 20% larger

bb  <- st_bbox(ind191)
# coerce to numeric to avoid weird attributes
...
cx  <- (xmin + xmax) / 2
cy  <- (ymin + ymax) / 2
dx  <- (xmax - xmin) * expand_factor / 2
dy  <- (ymax - ymin) * expand_factor / 2
...
# quick sanity check
bb_exp
# should be 4 finite numbers with xmin < xmax, ymin < ymax [web:45][web:48]

tmpl <- rast(xmin = bb_exp["xmin"], xmax = bb_exp["xmax"],
             ymin = bb_exp["ymin"], ymax = bb_exp["ymax"],
             resolution = 500,
             crs = st_crs(ind191)$wkt)   # [web:38]

v_ind191 <- vect(ind191)
count    <- rasterize(v_ind191, tmpl, fun = "length")   # [web:38][web:10]
count[is.na(count)] <- 0

plot(count, col = tim.colors(100))
mcp191 <- mcpData.proj[mcpData.proj$id == "X191", ]
plot(vect(mcp191), add = TRUE, border = "wheat", lwd = 2)
points(v_ind191, pch = 16, cex = 1, col="white")
points(v_ind191, pch = 1, cex = 1, col="grey")
