...
leroyInt <- filter(leroyInt, !st_is_empty(leroyInt)) # remove remaining NAs (first and last locations are empty and cannot be interpolated)
...
### Thinning/subsampling using move2
...
##### Experienting with doing the same with Leo, but work in progress
## first have a look at the distribution of timelags of your data. 
summary(mt_time_lags(Leo, units="min"))
## thin the data to 1 location per hour. Here the first location per 1h window is picked
Leo_5h <- mt_filter_per_interval(Leo,criterion="first",unit="5 hour")
tl <- mt_time_lags(Leo_5h, units="min")
summary(tl)
length(which(tl<set_units(300,"min")))/nrow(Leo_5h)

hist(sqrt(drop_units(mt_speed(Leo, units = "m/s"))))
hist(sqrt(drop_units(mt_speed(Leo_5h, units = "m/s"))))
