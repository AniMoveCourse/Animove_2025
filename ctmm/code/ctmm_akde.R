...
plot(DATA, col = COL, cex = 2, main = "Pepper")

SVF <- variogram(DATA, CI = "Gauss")
plot(SVF, fraction = 0.5, level = c(0.50, 0.95))
...
# save(FIT, file = "fits_buffalo_pepper.rda")
# I've already run this
load("data/fits_buffalo_pepper.rda")
...
load("data/akdes_buffalo.rda")

# color to be spatially distinct
COL <- color(AKDES, by = 'individual')

# plot AKDEs
plot(AKDES, col.UD = COL, col.level = COL,
     col.grid = NA, level = NA,
...
plot(buffalo, MEAN, col = COL, 
     main = "Mean African buffalo AKDE")
