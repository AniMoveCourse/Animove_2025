...
plot(buffalo_env[[1]], ext = extent(cilla_mv) * 1.5)
...
buffalo_env_list <- list("elev" = raster::raster(buffalo_env[["elev"]]),
           ...
           "ndvi_sd" = raster::raster(buffalo_env[["ndvi_sd"]]))

#' Definition of reference grid the akde predictions should get aligned to
reference_grid <- crop(buffalo_env_list[["elev"]], extent(cilla_mv) * 1.5)
...
m_rsf_cilla <- glm(case_*k_ ~ x_ + y_ + I(-(x_^2 + y_^2)/2) + elev + slope + ndvi_sd,
