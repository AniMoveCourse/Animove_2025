...
buffalo_env <- rast("/Users/bjoern/Dropbox/Projects/Animove_2025/Data/buffalo_env.tif") 
...
m_1 <- fit_clogit(ssf_cilla, case_ ~ cos(ta_) + sl_ + log(sl_) + slope_end + elev_end +
                    ndvi_mean_end + ndvi_sd_end + strata(step_id_))
summary(m_1)
...
round(cor(ssf_cilla[, c("slope_end", "elev_end",
  "water_dist_end", "ndvi_mean_end", "ndvi_sd_end")]), 2)
...
#' slope and ndvi_sd do not contribute significantly to the fit.
#'
#' ## Model selection
...
m_2 <- fit_clogit(ssf_cilla, case_ ~ cos(ta_) + sl_ + log(sl_) + slope_end + elev_end + ndvi_mean_end + strata(step_id_))
AIC(m_1$model)
AIC(m_2$model)
summary(m_2)

m_3 <- fit_clogit(ssf_cilla, case_ ~ cos(ta_) + sl_ + log(sl_) + elev_end + ndvi_mean_end + strata(step_id_))
AIC(m_3$model)
summary(m_3)
...
habitat_preference <- function(model, object, include_avail = TRUE, data_crs = crs(object)) {
  ...
  names(beta) <- gsub("_end", "", names(beta))
  if(include_avail) {
...
#' ## Iterate
#' Here: a model with time-varying preference for ndvi_mean
#' We group observation in 3 hour bins to smooth the picture
#'
boxplot(ndvi_mean_end ~ I(floor(hour/3)*3), data =
...
m_time_ndvi <- fit_clogit(ssf_cilla, case_ ~ cos(ta_) + sl_ + log(sl_) + elev_end + ndvi_mean_end +
                   ndvi_mean_end:pbs(hour, df = 5, Boundary.knots = c(0,24)) + strata(step_id_))
m_time_dist <- fit_clogit(ssf_cilla, case_ ~ cos(ta_) + sl_ + log(sl_) + elev_end + ndvi_mean_end +
...
pred_data_ndvi <- data.frame("step_id_" = stratum, ta_ = 0, sl_ = 1, elev_end = 0, ndvi_mean_end = 1, hour = seq(0, 24, len = 101))
m_time_ndvi_clogit <- clogit(case_ ~ cos(ta_) + sl_ + log(sl_) + elev_end + ndvi_mean_end +
                            ndvi_mean_end:pbs(hour, df = 5, Boundary.knots = c(0,24)) + strata(step_id_), data = ssf_cilla)
...
step_interval_simulation_amt <- lapply(buffalo_ids, function(animal) {
  lapply(step_durations, function(step_duration) {
    ...
    m_1 <- clogit(case_ ~ cos(ta_) + sl_ + log(sl_) + elev +
                    ndvi_mean + strata(step_id_), data = ssf_animal)
...
#' - Interaction with changes of internal states (coming from another model/other observations - or do HMM-SSF)
#'
